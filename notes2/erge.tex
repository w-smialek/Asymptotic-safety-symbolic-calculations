...
\end{equation}
% WCZEŚNIEJ O LINEAR SPLIT I ŻE WOKÓŁ FLAT BACKGROUND
...
(Let us note, that the fact which variational derivatives $\delta h$ and $\delta A$ of interaction term are taken, determines what
elements of $\mathcal{P}^{-1}$ later multiply it). In the diagramatical representation, this is equivalent to the fact that in any effective vertex two of the external legs are opened to form a loop,
while the rest is left uncontracted. This fact can be easily understood by representing the product $\mathcal{P}^{-1} \mathcal{F}$ diagramatically
\begin{figure}[H]
    \includegraphics[width=1\textwidth]{figures/drawing.png}
    \caption*{}
\end{figure}
Where a red line at the end of vertex leg denotes the functional derivative with respect to the corresponding field.

... 
A projection operator (jak go nazwałem wyżej) will then annihilate all terms not originating from diagrams with two external photon legs.
