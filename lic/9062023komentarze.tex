...
The Standard Model of particle physics is a unified description of all quantum fields observed in microscopic physics.  \jhkbf{What about gravity?}
...
In the pure \jhk{\sout{electroweak} electromagnetic} theory, one loop $\beta$-function of the abelian gauge coupling $g$ is: \cite{higgs mass}
\begin{equation}
    \beta_{g}^{\text{SM}} = \beta_{g}^{(3)} g^3 = \frac{41}{6} \frac{g_Y^3}{16 \pi^2},
\end{equation}
% \jhkbf{What is Y?}
This predicts a running of gauge coupling
\begin{equation}
    g(\mu) = \frac{g_{\text{obs}}}{\sqrt{1-2g_{\text{obs}}^2 \beta_g^{(3)}\log{\mu}}}
\end{equation}
that would diverge at a scale $\mu = \frac{\Lambda}{m_r} = \left(2 g_{\text{obs}}^2 \ \beta_{g}^{(3)} \right)^{-1} $.
...
\begin{figure}[H]
    ...
    \label{boxes}
\end{figure} 
Taking an electron mass as a reference scale, the location of abelian coupling landau pole \jhkbf{Musisz dać definicję.} will be of order $\Lambda \sim 10^{48} \ \text{eV}$, well above the Planck scale.
Similarly, quartic self-interaction of higgs field theoretically may exhibit a \jhk{landau} pole at a finite momentum scale.
