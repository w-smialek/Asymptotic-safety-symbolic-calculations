...
% inserted through adding a regulator term $\Delta S_k[\Phi]$ to the bare action $S[\Phi]$ in the definition of partition function
...
% is a functional which put in place of the bare action recreates the actual theory when all loop integrals are capped at momentum value $b\Lambda$, the effective action $\Gamma$
% includes the effect of all quantum fluctuations on effective vertices

% start from partition functional $Z[J]$ and separate
 
% the contribution from the field modes below certain threshold $p<b\Lambda$ and between it and the cutoff scale

% perform the functional integral over certain region of high momentum modes $b \Lambda \leq p \leq \Lambda$ and observe how the 
...
\begin{equation}
    ...
    % \int_0^{\infty} d(p^2) \ k^2 \ r(p^2/k^2) \ \hat{\mathcal{D}}_{p^2}, \qquad \hat{\mathcal{D}}_{p^2} \ \phi(x) e^{i q x} = \delta(p^2-q^2) \ \phi(x) e^{i q x}
\end{equation}
...
% but for $\hat{R}_k$ to acts as a proper IR regulator, it has to satisfy certain requirements.
